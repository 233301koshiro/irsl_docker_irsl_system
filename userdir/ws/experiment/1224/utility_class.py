...
class Graph:
    ...
    def save_graph_image(G, output_path, pos_flag):  # グラフ保存関数
        ...
        for node in G.nodes():
            ...
            if node_type == 'joint':
                color_map.append('blue')  # type='a' は青
            elif node_type == 'endeffector':
                color_map.append('red')  # type='b' は緑
            else:
                color_map.append('gray')  # デフォルト色

        # グラフ描画
        nx.draw(G, pos, node_size=100, node_color=color_map, with_labels=False)
    ...
    def remove_longest_edge_in_cycle_undirected(G,tf):#グラフ内のループを検出し、ループ内の最大の長さを削除
        ...
        print(f"サイクルを成すノードの座標: {cycle_node_positions}")
        max_edge = None
    ...
    def docking_graph(JG, EG,object_set,joint_set):#グラフの結合
        ...
        for eg_node, eg_pos in EG_pos.items():
            ...
            if(min(distances.values())<10):
                nodes_to_remove = [eg_node]  # eg_node を削除対象に追加
                GG.remove_nodes_from(nodes_to_remove)  # 指定したノードを削除
                print("エンドエフェクタとジョイントが近すぎるのでつなぎません",eg_pos,GG.nodes[min(distances, key=distances.get)]['pos'],min(distances.values()))
                continue
            # 最も近い末端ジョイントを選ぶ
            closest_jg_node = min(distances, key=distances.get)
            print("ジョイントとエンドエフェクタを結んだよ",GG.nodes[min(distances, key=distances.get)]['pos'],eg_pos,min(distances.values()))
            # エッジを追加し、新しいエッジを記録
